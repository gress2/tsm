...
optimizer = torch.optim.SGD(model.parameters(), lr=1e-8, momentum=.7, weight_decay=2e-5)
bs = 32
...
for epoch in range(int(3)):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        for j in range(len(x)):
            yj_actual_sz = k[j].type(torch.LongTensor)
            yj = y[j][0 : yj_actual_sz]
            ll1 = torch.lgamma(k[j] * epsilon[j])
            ll2 = -1 * k[j] * torch.lgamma(epsilon[j])
            ll3 = (epsilon[j] - 1) * torch.sum(torch.log(yj + 1e-10))
